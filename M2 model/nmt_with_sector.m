function [mito]=nmt_with_sector(principalmat)
...
fu(1,1)=0.017/(2*(5)^2);fu(1,2)=0.017/((5)^2);%stores the fusion 
                       %frequencies in the presence of microtubule(wmt)
fu(2,1)=0.011/(2*(5)^2);fu(2,2)=0.011/((5)^2);%stores the fusion frequencies in the aabsence of microtubule(no mt)
fi(1,1)=0.018/5;fi(1,2)=2*0.018/5;%stores the fission frquencies in the presence of microtubles(wmt)
fi(2,1)=0.027/5;fi(2,2)=2*0.027/5;%stores the fission frquencies in the absence of microtubules(no mt)
...
Mltc=sum(ffes,3);
mito=Mltc;
%mito stores the averaged number of mitochondria present in each fragment
