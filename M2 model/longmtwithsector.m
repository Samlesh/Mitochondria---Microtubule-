...
for k=1:15
    ...
    % being stored
    Mlilswiths(:,:,k)=lmt_with_sector(stormatrix(:,:,k));
    %lmt_with_sector returns the number of mitochondria at every time
    %obtained from simulation using the time=0 data from the
    %stormatrix(:,:,k) for the kth cell
toc
end
save('longmtwithsector.mat','Mlilswiths');
%Mlilswiths saves the resulatant mitochondrial number with time in the matrix
%longmtwithsector.mat