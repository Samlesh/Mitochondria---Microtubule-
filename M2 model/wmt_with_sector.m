function [mitochondria]=wmt_with_sector(principalmat)
%principalmat -- the matrix which consists of the number of mitochondria
            %present in each fragement at various time steps in experiment
...
fu(1,1)=0.017/(2*(4)^2);fu(1,2)=0.017/((4)^2);%stores the fusion 
                       %frequencies in the presence of microtubule(wmt)
fu(2,1)=0.011/(2*(4)^2);fu(2,2)=0.011/((4)^2);%stores the fusion 
                         %frequencies in the absence of microtubule(no mt)
fi(1,1)=0.018*0.05/4;fi(1,2)=2*0.018*0.05/4;%stores the fission frequencies
                                   %in the presence of microtubles(wmt)
fi(2,1)=0.027/4;fi(2,2)=2*0.027/4;%stores the fission frequencies
