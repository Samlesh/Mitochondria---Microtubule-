...
%Almt stores the mitochondrial length values observed in experiments for 
...
%38 is the mean number of mitochondria acros all cells at time t=0
initiallmt=stormatrix(:,:,10);%stores the time evolution of cell 5
n0=initiallmt(:,1);%stores the initial condition
...
fu(1,1)=0.011/(2*(38)^2);fu(1,2)=0.011/((38)^2);%stores the fusion 
                       %frequencies in the presence of microtubule(long mt)
fu(2,1)=0.011/(2*(38)^2);fu(2,2)=0.011/((38)^2);%stores the fusion 
                         %frequencies in the absence of microtubule(no mt)
fi(1,1)=0.006/38;fi(1,2)=2*0.006/38;%stores the fission frequencies
                                   %in the presence of microtubles(long mt)
fi(2,1)=0.027/38;fi(2,2)=2*0.027/38;%stores the fission frequencies
...
for s=1:numel(initialc(1,:))
   ...
   %n0 is the initial condition of a particular sector 
    ...
    if sectornumber==1
        mt_ldata=a1l;
        %a1l-length of microtubules in sector 1
    elseif sectornumber==2
        mt_ldata=a2l;
        %a2l-length of microtubules in sector 2
    end
...
Mltc10=sum(ffes,3);
%Mltc10 stores the averaged number of mitochondria present in each fragment
%corresponding to vaious time steps and the averaging is done across the
%realizations
toc
save('cell10sectorlmt','Mltc10','-v7.3');
else
