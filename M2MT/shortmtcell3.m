...
%Asmt stores the mitochondrial length values o bserved in experiments for 
...
% fragments
for k=1:8
    %there are 8 cells in the experiments having long microtubules 
...
%54 is the mean number of mitochondria acros all cells at time t=0
initialsmt=stormatrix(:,:,3);%stores the time evolution of cell 3
n0=initialsmt(:,1);%stores the initial condition
...
fu(1,1)=0.025/(2*(54)^2);fu(1,2)=0.025/((54)^2);%stores the fusion 
                       %frequencies in the presence of microtubule(short mt)
fu(2,1)=0.011/(2*(54)^2);fu(2,2)=0.011/((54)^2);%stores the fusion 
                         %frequencies in the absence of microtubule(no mt)
fi(1,1)=0.032/54;fi(1,2)=2*0.032/54;%stores the fission frequencies
                                   %in the presence of microtubles(short mt)
fi(2,1)=0.027/54;fi(2,2)=2*0.027/54;%stores the fission frequencies
...
for s=1:numel(initialc(1,:))
    ...
    if sectornumber==1        
        mt_ldata=a1s;
        %a1s-length of microtubules in sector 1
    elseif sectornumber==2
        mt_ldata=a2s;
     %a2s-length of microtubules in sector 2
    end
    for i=1:500
...
Mstc3=sum(ffes,3);
%Mstc3 stores the averaged number of mitochondria present in each fragment
%corresponding to vaious time steps and the averaging is done across the
%realizations
toc
save('cell3sectorsmt','Mstc3','-v7.3');
else
